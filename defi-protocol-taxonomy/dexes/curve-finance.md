...
Their Frontend, Dune, DefiPulse - to be provided. DefiLlama - it is calculated by counting the tokens locked on AMM pools, pulling the data from the respective subgraphs mentioned here \[24].
...
Curve Finance from a LP provider perspective gives the HODL exposure, generated fees and Impermanent Loss (IL).Impermanent loss is more if price movement is more as curve deals with stable coin pools.So impermanent loss is generally less in Curve Finance. Below is the IL(percentage) curve for relativistic price changes \[25].
...
No, Curve doesn't automatically wrap its assets however if they are associated with lending then it will automatically wrap it.&#x20;
...
There are no liquidations in case of Curve but assets can become idle.If one of the stablecoins in the pool goes significantly down and never returns to the original value, it'll effectively mean that pool liquidity providers hold almost all their liquidity in that currency.\[26]

### **12. What are the dependencies of the protocol? How does risk bubble up?**

Curve does not has any dependencies on any third party apps or APIs.
...
veCRV stands for vote-escrowed CRV, a fancy name for CRV that’s been locked into the Curve protocol for a defined time. veCRV is used for voting in governance, boosting rewards, earning trading fees, and receiving airdrops \[13].

### **14.** How decentralised
...
* **Decision Making** \[15]&#x20;
  ...
  * Using the signaling tool is completely free (no transaction fees) and you only need 1veCRV to create a proposal there.&#x20;
...
* **LCR** - Not applicable&#x20;
* **Are there any other metrics that might be useful?Like Sharpe Ratio Sharpe Ratio** - could be useful when applied to the UNI token market behaviour&#x20;
